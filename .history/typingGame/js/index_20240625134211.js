document.addEventListener('DOMContentLoaded', function () {
    ...
    const repetitionsSelect = document.querySelector('#repetitions');
    ...
    const paragraphs = {
        ...
        3: `બકમા વછીસ બકમા વછીસ બકમા વછીસ બકમા વછીસ`,
    ...
    const repeatLesson = (lessonText, repetitions) => {
        return Array(repetitions).fill(lessonText).join(' ');
    };

    if (lesson && paragraphs[lesson]) {
        const repetitions = parseInt(repetitionsSelect.value, 10);
        sentenceToWrite = repeatLesson(paragraphs[lesson], repetitions).split(' ');
        showSentence.innerHTML = repeatLesson(paragraphs[lesson], repetitions);
    } else {
    ...
    repetitionsSelect.addEventListener('change', () => {
        const repetitions = parseInt(repetitionsSelect.value, 10);
        sentenceToWrite = repeatLesson(paragraphs[lesson], repetitions).split(' ');
        showSentence.innerHTML = repeatLesson(paragraphs[lesson], repetitions);
    });
