...
const calculateTypingSpeed = (time_taken) => {
    let totalWords = typing_ground.value.trim();
    let actualWordsArray = totalWords === '' ? [] : totalWords.split(/\s+/);
    let markedText = errorChecking(actualWordsArray);

    if (actualWordsArray.length > 0) {
        let correctWords = actualWordsArray.filter(word => !word.startsWith('<span style="color: red">'));

        if (correctWords.length !== 0) {
            let typing_speed = (correctWords.length / time_taken) * 60;
            typing_speed = Math.round(typing_speed);
            score.innerHTML = `Your typing speed is ${typing_speed} words per minute & you wrote ${correctWords.length} correct words out of ${sentence_to_write.length} words & time taken ${time_taken} sec`;
        } else {
            score.innerHTML = `Your typing speed is 0 words per minute & time taken ${time_taken} sec`;
        }
    } else {
    ...
    typing_ground.innerHTML = markedText;
};
...
const errorChecking = (userWords) => {
    let markedWords = [];
    sentence_to_write = show_sentence.textContent.trim().split(/\s+/);

    for (let i = 0; i < userWords.length; i++) {
        if (userWords[i] === sentence_to_write[i]) {
            markedWords.push(`<span>${userWords[i]}</span>`);
        } else {
            markedWords.push(`<span style="color: red">${userWords[i]}</span>`);
        }
    }
