...
const typing_ground = document.querySelector('#typing_ground');
const btn = document.querySelector('#btn');
const score = document.querySelector('#score');
const show_sentence = document.querySelector('#showSentence');
const show_time = document.querySelector('#show-time'); 
...
const errorChecking = (userWords) => {
    ...
    for (let i = 0; i < sentence_to_write.length; i++) {
        if (userWords[i] === sentence_to_write[i]) {
            highlightedWords.push(sentence_to_write[i]);
            num++;
        } else {
            highlightedWords.push(`<span class="highlight-wrong">${userWords[i] || ''}</span>`);
        }
    }

    show_sentence.innerHTML = highlightedWords.join(' ');
    return num;
};
