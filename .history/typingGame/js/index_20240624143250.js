document.addEventListener('DOMContentLoaded', function () {
    ...
    const errorChecking = (userWords) => {
        ...
        // Track the index of the last correct word in the user input
        let lastCorrectIndex = -1;

        for (let i = 0; i < sentenceToWrite.length; i++) {
            // Check if user has word at the current position
            if (userWords[i - missingWords] !== undefined) {
                if (userWords[i - missingWords] === sentenceToWrite[i]) {
                    // Update the index of the last correct word
                    lastCorrectIndex = i;
                    numCorrect++;
                } else {
                    incorrectWords.push(userWords[i - missingWords]);
                }
            } else {
                missingWords++;
            }
        ...
        // Adjust incorrect words array to match the original indexes
        if (missingWords > 0 && incorrectWords.length > 0) {
            incorrectWords = incorrectWords.slice(lastCorrectIndex + 1);
        }
