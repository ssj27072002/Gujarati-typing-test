document.addEventListener('DOMContentLoaded', function () {
    ...
    btn.addEventListener('click', function () {
        if (this.textContent === 'Start') {
            ...
            // Display each word in the sentence with correct/incorrect styling
            ...
            // Enable the typing area for user input
            typingGround.disabled = false;
            typingGround.focus();
        } else if (this.textContent === 'Check') {
            this.textContent = 'Restart';
            endTime = new Date();
            totalTimeTaken = Math.floor((endTime - startTime) / 1000);

            // Calculate words per minute (WPM)
            let wordsPerMinute = Math.round((correctWords / totalTimeTaken) * 60);

            // Display the score
            score.textContent = `Your typing speed: ${wordsPerMinute} WPM`;
    ...
    typingGround.addEventListener('keydown', function (event) {
        if (event.key === 'Backspace') {
            backspaceCount++;
        }
    });

    // Display backspace count if needed
    typingGround.addEventListener('blur', function () {
        if (backspaceCount > 0) {
            score.textContent += ` | Backspace used: ${backspaceCount}`;
        }
    });
