...
const errorChecking = (userWords) => {
    ...
    for (let i = 0; i < userWords.length; i++) {
        if (userWords[i] === sentence_to_write[i]) {
            result.push({ word: userWords[i], correct: true });
        } else {
            result.push({ word: userWords[i], correct: false });
        }
    }
    return result;
};
...
const endTypingTest = () => {
    ...
    const userWords = typing_ground.value.trim().split(/\s+/);
    const errors = errorChecking(userWords);
    ...
    errors.forEach((item) => {
        if (item.correct) {
            ...
            html += `<span style="color: red;">${item.word}</span> `;
        }
    });

    typing_ground.innerHTML = html;
    score.innerHTML = `Your typing speed is ${typing_speed} words per minute & you wrote ${correctWords} correct words out of ${sentence_to_write.length} words & time taken ${time_taken} sec`;
};
