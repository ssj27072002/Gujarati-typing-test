document.addEventListener('DOMContentLoaded', function () {
    ...
    const errorChecking = (userWords) => {
        ...
        let j = 0;
        let i = 0;
        while (i < sentenceToWrite.length && j < userWords.length) {
            if (userWords[j] === sentenceToWrite[i]) {
                numCorrect++;
            } else {
                incorrectWords.push(userWords[j]);
                // If the next word in user input is not equal to the expected word, skip to the next expected word
                while (i < sentenceToWrite.length && userWords[j] !== sentenceToWrite[i]) {
                    missingWords++;
                    i++;
                }
            }
        ...
        // If there are remaining words in the original text, count them as missing
        missingWords += sentenceToWrite.length - i;
