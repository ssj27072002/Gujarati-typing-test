document.addEventListener('DOMContentLoaded', function () {
    ...
    const paragraphs = {
        ...
        3: `આજે મારા મિત્રો ભેગા થયા છે. અમે મળીને રમ્યા અને મજા કરી.`,
    ...
    const hints = {
        1: {
            ...
            'ષ': 'sh', 'સ': 's', 'હ': 'h', 'ળ': 'l', 'ક્ષ': 'ksh',
            'જ્ઞ': 'gyan'
        },
        2: {
            'અ': 'a', 'આ': 'aa', 'ઇ': 'i', 'ઈ': 'ii', 'ઉ': 'u', 'ઊ': 'uu',
            'ઋ': 'ru', 'એ': 'e', 'ઐ': 'ai', 'ઓ': 'o', 'ઔ': 'au',
            'ક': 'k', 'કા': 'ka', 'કી': 'ki', 'કુ': 'ku', 'કૂ': 'kuu', 'કે': 'ke', 'કૈ': 'kai', 'કો': 'ko', 'કૌ': 'kau', 'કં': 'kam', 'કઃ': 'kah'
        },
        // Add hints for other lessons here
    };

    if (lesson && paragraphs[lesson]) {
        sentenceToWrite = paragraphs[lesson].split(' ');
        showSentence.innerHTML = paragraphs[lesson];
        hint.innerHTML = '';  // Clear hint area at the start
    } else {
    ...
    typingGround.addEventListener('input', () => {
        ...
        hint.innerHTML = hints[lesson][currentWord] || '';
