document.addEventListener('DOMContentLoaded', function () {
    ...
    typingGround.addEventListener('input', (event) => {
        ...
        const targetWord = sentenceToWrite[currentWordIndex];

        const isCorrect = currentWord === targetWord;

        if (isCorrect) {
            correctWords++;
            typingGround.value = words.slice(0, currentWordIndex + 1).join(' ') + ' ';
            currentWordIndex++;
        }

        updateTextColor(typingGround, words, sentenceToWrite);
    ...
    const updateTextColor = (inputField, userWords, targetWords) => {
        const coloredText = userWords.map((word, index) => {
        ...
        inputField.innerHTML = coloredText;
    };
