...
const endTypingTest = () => {
    ...
    sentence_to_write.forEach((word, index) => {
        if (incorrectPositions.includes(index)) {
            sentence_to_write[index] = `<span class="error">${word}</span>`;
        }
    });

    show_sentence.innerHTML = sentence_to_write.join(' ');
    typing_ground.value = "";
};
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const highlightErrors = () => {
        let highlightedText = sentenceToWrite.map((word, index) => {
            if (incorrectPositions.includes(index)) {
        ...
        showSentence.innerHTML = highlightedText;
    };
