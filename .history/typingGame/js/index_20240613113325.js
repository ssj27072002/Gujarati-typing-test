...
const errorChecking = (userWords) => {
    let numCorrect = 0;
    sentence_to_write = show_sentence.innerHTML.trim().split(/\s+/);
    typing_ground.value = ''; // Clear previous content
    typing_ground.innerHTML = ''; // Clear previous content

    for (let i = 0; i < userWords.length; i++) {
        if (userWords[i] === sentence_to_write[i]) {
            numCorrect++;
            typing_ground.innerHTML += userWords[i] + ' ';
        } else {
            typing_ground.innerHTML += `<span class="error">${userWords[i]}</span> `;
        }
    }
    return numCorrect;
};
